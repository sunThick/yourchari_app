...
class ChariLikeController extends ChangeNotifier {
  ...
  Future<void> like(
      ...
      required DocumentReference<Map<String, dynamic>> chariRef,
      required MainController mainController}) async {
    // setting
    final String postId = chari.postId;
    mainController.likeChariIds.add(postId);
  ...
  Future<void> unlike(
    ...
    final QuerySnapshot<Map<String, dynamic>> qshot = await FirebaseFirestore
        ...
        .doc(activeUid)
        .collection("tokens")
        .where("postId", isEqualTo: chari.postId)
        .get();
